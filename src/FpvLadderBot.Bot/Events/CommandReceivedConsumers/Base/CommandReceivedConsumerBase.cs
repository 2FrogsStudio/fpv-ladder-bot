namespace FpvLadderBot.Events.CommandReceivedConsumers.Base;

public abstract class CommandReceivedConsumerBase(
    Command command,
    ITelegramBotClient bot,
    IScopedMediator mediator)
    : IMediatorConsumer<CommandReceived> {
    ...
    public async Task Consume(ConsumeContext<CommandReceived> context) {
        ...
        Response<GetBackNavigationResult[], EmptyNavigation> response = await mediator
        ...
        if (menuMessageId.HasValue) {
            await bot.EditMessageText(
                ...
                InlineKeyboard is not null ? new InlineKeyboardMarkup(InlineKeyboard) : null,
